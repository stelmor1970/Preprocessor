...
4. 항목 설명: 두문자 각 글자를 앞에 **bold**로 표시하고 내용 서술
   - 수치/숫자는 반드시 포함
   - 단서/예외 조건은 들여쓰기 sub-항목으로
   (예: **십**: 10층 이하의 층은 바닥면적 1천㎡ 이내마다 구획 (스프링클러 등 자동식 소화설비 설치 시 3천㎡))

5. 표: 비교 기준이 있으면 표로 정리
   (예: 방화문 구분 — 구분/차염성능/차열성능 3열 표)
   | 구분 | 항목A | 항목B |
   |------|-------|-------|
   | 내용 | 값    | 값    |

6. Strategist's Tip: 시험 함정 또는 출제 패턴 경고
   (예: [Strategist's Tip] 자동방화셔터의 설치 거리(3m)는 시험에서 5m로 자주 바꿔치는 단골 함정입니다.)
