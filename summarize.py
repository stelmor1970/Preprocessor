...
수험서의 내용을 분석하여 수험생이 시험에 합격할 수 있도록 핵심 내용을 정리해 주세요.
...
## 암기 포인트
- 시험에 자주 출제되는 수치, 기준, 규정 등

## 주의사항
- 혼동하기 쉬운 내용이나 예외사항

각 항목은 간결하고 명확하게 작성하고, 수험생이 쉽게 암기할 수 있도록 구조화해 주세요."""
...
def summarize_chunk(client: anthropic.Anthropic, chunk: list[dict], chunk_num: int, total_chunks: int) -> str:
    ...
    with client.messages.stream(
        ...
        messages=[
            {
                "role": "user",
                "content": f"다음은 수험서의 페이지 {start_page}~{end_page} 내용입니다. 핵심 내용을 요약 정리해 주세요.\n\n{content}"
            }
        ]
    ) as stream:
...
요구사항:
1. 전체 내용의 핵심을 체계적으로 정리
2. 출제 빈도가 높은 항목 강조
3. 챕터/주제별로 구조화
4. 수험생이 최종 점검용으로 활용할 수 있는 형태
