...
**【모범답안】**
채점자가 점수를 줄 수 있는 핵심 키워드와 내용을 중심으로 요약 정리합니다.
- 채점 포인트가 되는 핵심 용어·수치는 **bold** 처리
- 항목이 여럿이면 번호 목록으로 구조화
- 수치(㎡, m, 층, 분 등)와 "이상/이하/미만/초과" 표현은 반드시 포함
- 단서·예외 조건은 들여쓰기 sub-항목으로 표현
- 괄호 안 조건(완화기준 등)도 반드시 포함
- 법령 근거가 있으면 조·항·호 포함
- 비교 항목이 2개 이상이면 마크다운 표로 정리
작성 기준: 수험생이 이 내용만 외워서 써도 부분점수 이상을 받을 수 있어야 합니다.

**【보충정보】**
시험에 나올 수 있는 추가 개념·정의·판단기준을 핵심만 요약합니다.
- 원문의 부연 설명 중 시험 출제 가능성이 높은 내용만 선별
- 용어 정의는 채점 키워드 중심으로 압축
- 수치·조건은 생략 없이 포함
...
--- 절대 규칙 ---
1. 【문제】는 서술형 지시어만 의문형으로 바꾸고 법령명·주제어는 그대로 유지합니다
2. 【모범답안】·【보충정보】는 채점 키워드 중심으로 요약하되 다음은 절대 생략 금지:
   - 수치(㎡, m, 층, 분, 개 등)와 이상/이하/미만/초과 표현
   - ( ) 괄호 안 조건·단서·완화기준 (예: "1,000㎡(스프링클러 시 3,000㎡)")
   - 법률명·시행령·시행규칙·고시 명칭 및 조·항·호 번호
   - 예외·면제·완화 조건
   - 용어 정의의 핵심 키워드
3. 두문자 암기전략은 내용을 '대체'하는 것이 아니라 맨 마지막에 '추가'하는 것입니다
4. 이미지에서 표가 보이면 반드시 마크다운 표로 변환합니다
5. 필기 메모·밑줄·강조 표시도 시험 출제 가능성이 높은 중요 정보로 포함합니다
6. 채점 기준: 수험생이 【모범답안】만 외워 써도 부분점수 이상을 받을 수 있어야 합니다"""
...
def summarize_chunk(
    ...
    image_blocks.append({
        ...
        "text": (
            ...
            "【절대 규칙】\n"
            "- 【문제】: 법령명·주제어 유지, 문말 지시어만 의문형으로 변환 (쓰시오→은?)\n"
            "- 【모범답안】·【보충정보】: 채점 키워드 중심으로 요약 (원문 그대로 X, 핵심만 O)\n"
            "- 수치·괄호 안 조건·법령명·조항 번호·예외·완화조건은 절대 생략 금지\n"
            "- 채점 기준: 수험생이 이 내용만 써도 부분점수 이상을 받을 수 있어야 합니다\n"
            "- 두문자 암기전략은 맨 마지막에 추가하는 것이며 내용을 대체하지 않습니다\n"
            "- 【핵심요약】을 반드시 작성합니다: 출제 포인트와 핵심 수치·조건을 한 문장으로\n\n"
            "각 문제를 system prompt 양식에 따라:\n"
            "【문제】→【핵심요약】→【모범답안】→【보충정보】→【두문자 암기전략】→【Strategist's Tip】 순서로 작성하십시오.\n\n"
